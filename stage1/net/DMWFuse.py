...
class WaveletFeatureExtractor(nn.Module):
    ...
    def forward(self, x):
        ...
        high_freq = x - F.interpolate(LL, scale_factor=2, mode='bilinear', align_corners=False)  
        return LL, high_freq
...
class TransformerBlock(nn.Module):
    def __init__(self, dim, num_heads, ffn_expansion_factor, bias, LayerNorm_type):
        ...
        self.wavelet_extractor = WaveletFeatureExtractor(wavelet_type='haar')
        self.wavelet_attention = WaveletAttentionFusion(dim=dim, num_heads=num_heads)

    def forward(self, x):
        # 小波变换增强
        LL, high_freq = self.wavelet_extractor(x)
        high_freq = self.wavelet_attention(high_freq)
...
class DMW_Fuse(nn.Module):
    ...
    def forward(self, inp_img,noise_emb = None):
        ...
        B = inp_img.shape[0]  
        num_timesteps = 1000  
        t = torch.randint(0, num_timesteps, (B,), device=inp_img.device).float().unsqueeze(1)  
        task_harmonization_output1, loss_tmp = self.degradmoe1(inp_enc_level1, task_prompt, t)
        loss_b = loss_tmp
        out_enc_level1 = self.encoder_level1(task_harmonization_output1)
        inp_enc_level2 = self.down1_2(out_enc_level1) 
        task_harmonization_output2, loss_tmp = self.degradmoe2(inp_enc_level2, task_prompt, t)  
        loss_b = loss_tmp + loss_b
        out_enc_level2 = self.encoder_level2(task_harmonization_output2)
        inp_enc_level3 = self.down2_3(out_enc_level2)
        task_harmonization_output3, loss_tmp = self.degradmoe3(inp_enc_level3, task_prompt, t)  
