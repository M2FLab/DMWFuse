...
class Fusionloss(nn.Module):
    ...
    def forward(self,image_vis,generate_img):
        image_y,generate_img = image_vis,self._rgb2gray(generate_img)
        x_in_max=image_y
        #loss_in=nn.L1Loss()(x_in_max,generate_img).cuda('cuda:0')
        y_grad=self.sobelconv(image_y)
        #ir_grad=self.sobelconv(image_ir)
        generate_img_grad=self.sobelconv(generate_img)
        x_grad_joint=y_grad
        y_laplacian            = self.laplacianconv(image_y)
        generate_img_laplacian = self.laplacianconv(generate_img)
        
        loss_laplacian         = nn.L1Loss()(y_laplacian,generate_img_laplacian).cuda('cuda:0')
        loss_grad=nn.L1Loss()(x_grad_joint,generate_img_grad).cuda('cuda:0')
        return loss_grad, loss_laplacian 
