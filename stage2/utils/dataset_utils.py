...
class TrainDataset(Dataset):
    def __init__(self, args):
        ...
        print(self.de_type)
        self.de_dict = {'denoise15': 0,'denoise25': 1,'denoise50': 2, 'dehaze': 3,  'defocusblur': 4, 'stripe': 5}
        self._init_ids()
        self._merge_ids()
    ...
    def _init_ids(self):
        ...
        if 'dehaze' in self.de_type:
            self._init_hazy_ids()
        if 'defocusblur' in self.de_type:
    ...
    def _crop_patch(self, img_1, img_2, img_3, img_4):
        H = img_1.shape[0]
        W = img_1.shape[1]
        ...
        patch_4 = img_4[ind_H:ind_H + self.args.patch_size, ind_W:ind_W + self.args.patch_size]
    ...
    def _get_nonstripergb_name(self, stripe_name):
        gt_name = stripe_name.replace("degrad", "visible")
        return gt_name
    ...
    def _get_denoiseir_name(self, noise_name):
        gt_name = noise_name.replace("degrad", "infrared")
        return gt_name

    def _get_nonhazyir_name(self, dehazegt_name):
        gt_name = dehazegt_name.replace("degrad", "infrared")
        return gt_name

    def _get_deblurir_name(self, deblur_name):
        gt_name = deblur_name.replace("degrad", "infrared")
        return gt_name
    ...
    def _merge_ids(self):
        ...
        if "motiondeblur" in self.de_type:
            self.sample_ids += self.motiondeblur_ids
        if "defocusblur" in self.de_type:
    ...
    def __getitem__(self, idx):
        ...
        if de_id == 0:
            ...
            ir_name = self._get_denoiseir_name(sample["clean_id"])
            ir_img = crop_img(np.array(Image.open(ir_name).convert('L')), base=16)
            ir_clean = ir_img
        ...
        if de_id == 2:
            ...
            clean_img = crop_img(np.array(Image.open(clean_name).convert('L')), base=16)
            ir_name = self._get_nonhazyir_name(sample["clean_id"])
            ir_img = crop_img(np.array(Image.open(ir_name).convert('L')), base=16)
            ir_clean = ir_img
            ...
            clean_img = crop_img(np.array(Image.open(clean_name).convert('L')), base=16)
            ir_name = self._get_deblurir_name(sample["clean_id"])
            ir_img = crop_img(np.array(Image.open(ir_name).convert('L')), base=16)
            ir_clean = ir_img
        elif de_id == 5:
            ir_img = crop_img(np.array(Image.open(sample["clean_id"]).convert('L')), base=16)
            clean_name = self._get_nonstripergb_name(sample["clean_id"])
            degrad_img = crop_img(np.array(Image.open(clean_name).convert('L')), base=16)
            ir_name = self._get_nonstripe_name(sample["clean_id"])
            ir_clean = crop_img(np.array(Image.open(ir_name).convert('L')), base=16)
            clean_img = degrad_img

        degrad_patch, clean_patch, ir_patch, ir_clean = random_augmentation(*self._crop_patch(degrad_img, clean_img, ir_img, ir_clean))    
        degrad_patch, clean_patch, ir_patch, ir_clean  = self.toTensor(degrad_patch), self.toTensor(clean_patch), self.toTensor(ir_patch), self.toTensor(ir_clean)
        return [clean_name, de_id], degrad_patch, clean_patch, ir_patch, ir_clean
...
class TestDataset(Dataset):
    ...
    def _init_input_ids(self):
        ...
        if self.task_idx == 1:
            ...
            self.ids = []
            name_list = os.listdir(self.args.stripe_path + 'visible/')
            self.ids += [self.args.stripe_path + 'visible/' + id_ for id_ in name_list]
    ...
    def _get_gt_path(self, degraded_name):
        if self.task_idx == 0:
            ...
            gt_name = degraded_name.replace("visible", "visible")

        return gt_name


    def _get_ir_path(self, degraded_name):
        if self.task_idx == 0:
            ...
            gt_name = degraded_name.replace("visible", "input")
    ...
    def __getitem__(self, idx):
        ...
        ir_id = self._get_ir_path(degraded_path)
        ...
        ir_img = crop_img(np.array(Image.open(ir_id).convert('L')), base=16)
        ir_clean = crop_img(np.array(Image.open(ir_id.replace('input','infrared')).convert('L')), base=16)
        degraded_img = crop_img(np.array(Image.open(degraded_path).convert('L')), base=16)
        clean_img, degraded_img, ir_img, ir_clean = self.toTensor(clean_img), self.toTensor(degraded_img), self.toTensor(ir_img), self.toTensor(ir_clean)

        return [degraded_name], degraded_img, clean_img, ir_img, ir_clean, data_YCbCr2, gray
